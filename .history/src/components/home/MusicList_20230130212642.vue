<template>
  <div class="musiclist">
    ...
    <div class="musiccontent">
        <div class="list" v-for="l in ongList" :key="l.id">
            <img :src="l.picUrl">
            <div class="nums">
                {{l.playCount}}
            </div>
...
export default {
    ...
    setup(){
        ...
        // 计算属性
        let ongList = computed(()=>{
            obj.songList.forEach((value)=>{
                let count = value.playCount
                if(count>=100000000){
                    obj.songList.playCount =  (count/100000000).toFixed(1) + "亿"
                }
                else if(count>=10000){
                    obj.songList.playCount = (count/10000).toFixed(1) + "万"
                }
            })
            return obj.songList
        })
